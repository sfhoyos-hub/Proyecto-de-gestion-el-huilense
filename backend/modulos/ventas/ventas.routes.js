...
router.post('/', async (req, res) => { 
    try { 
        ...
        const ventaExistente = await db.get( 
            'SELECT * FROM ventas WHERE mesero_id = ? AND domingo_id = ?', 
            [mesero_id, domingo.id] 
        ); 
        ...
        const resultado = await db.run( 
            `INSERT INTO ventas (mesero_id, domingo_id, total_vendido) 
             VALUES (?, ?, ?)`,
            [mesero_id, domingo.id, monto]
        ); 
        ...
        if (fecha) { 
            ...
            ventas = await db.all( 
                `SELECT v.id, 
                        ...
                        m.identificacion 
                 FROM ventas v 
                 JOIN domingos d ON d.id = v.domingo_id
